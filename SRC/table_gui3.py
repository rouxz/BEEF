...
class MyTableView(QTableView):
	...
	def setDataConsistency(self):
		...
		# 5 - Update RASK and LF from AllFlow tab
		self.parent.updateDataFromAllFlows()
	
	def setDataConsistencyRASK(self, tabs):
		# calculate RASK
		...
		for r in VERTICAL_HEADER:
			...
			if regexp8.match(r) != None:
				...
				for i in xrange(0,17):
					...
					for tab in tabs:
						if tab.flow == "All":
							if tab.tableModel.getDataFloat(VERTICAL_HEADER.index("ASK" + fin),i) > 0:
								DataAllFlowRASK = tab.tableModel.getDataFloat(VERTICAL_HEADER.index("Rev" + fin),i) /  tab.tableModel.getDataFloat(VERTICAL_HEADER.index("ASK" + fin ),i)
					# if re.compile("(RASK).(AY).(Ref).*").match(r) != None:
						# print("-----------------------fin = "+ str(fin) + "    self.flow = " + str(self.flow) + "    mois = " + str(i) + "  variable = "+ str(r)+ "  DataAllFlowRASK = "+str(DataAllFlowRASK))
					self.tableModel.setDataNoDisplayUpdate(VERTICAL_HEADER.index(r), i , DataAllFlowRASK)
		...
		for r in VERTICAL_HEADER:
			...
			if regexp9.match(r) != None:
				fin = r[0:7]
				print("fin = "+ str(fin) + "    self.flow = " + str(self.flow)  )
				for i in xrange(0,17):
					denum = self.tableModel.getDataFloat(VERTICAL_HEADER.index(r[0:7] + " Ref"),i)
					if denum > 0:
						# print("Mois " + str(i) + " " + str(denum) + " " + str(self.tableModel.getDataFloat(VERTICAL_HEADER.index(r[0:7] + " CY"),i)))
						self.tableModel.setDataNoDisplayUpdate(VERTICAL_HEADER.index(r), i , ((self.tableModel.getDataFloat(VERTICAL_HEADER.index(r[0:7] + " CY"),i) / denum) - 1) * 100)
					else:
						self.tableModel.setDataNoDisplayUpdate(VERTICAL_HEADER.index(r), i , 1)
